...
namespace LineWatch
{
    ...
    public class PLC(string Name, string IP, string Printer, bool Label) : S7Client
    {
        ...
        private new void Connect()
        {
            ...
            else
            {
                if (IsOKbefore == true)
                {
                    File.AppendAllText(@"./failures/" + Name + ".txt", DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss ") + Name + " " + ErrorText(res) + "\n\r");
            ...
            while (true)
            {
                try
                {
                    ...
                    else
                    {
                        if (Connected)
                        {
                            //Обновляем статус линии в БД.
                            await DataAccess.UpdateIsOnline(Name, true);
                            IsOKbefore = true;
                            //Посылаем лайвбит
                            ...
                            if (result == 0)
                            {
                                ...
                                else
                                {
                                    if (IsOKbefore == true)
                                    {
                                        ...
                                        File.AppendAllText(@"./failures/" + Name + ".txt", DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss ") + Name + "  Ошибка чтения блока 1012: " + ErrorText(result) + "\n\r");
                                        IsOKbefore = false;
                                    }
                                }
                            }
                            else
                            {
                                if (IsOKbefore == true)
                                {
                                    ...
                                    File.AppendAllText(@"./failures/" + Name + ".txt", DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss ") + Name + "  Ошибка записи лайвбита: " + ErrorText(result) + "\n\r");
                                    IsOKbefore = false;
                                }
                            }
                        }
                        ...
                        {
                            //Обновляем статус линии в БД.
                            await DataAccess.UpdateIsOnline(Name, false);
                            Connect();
                        }
                    }
                    Thread.Sleep(100);
                ...
                {
                    Console.WriteLine(DateTime.Now.ToString() + " " + Name + " Ошибка в главном цикле станции: " + Name+ " - " + ex.Message);
                    File.AppendAllText(@"./failures/" + Name + ".txt", DateTime.Now.ToString() + " " + Name + " Ошибка в главном цикле станции: " + Name + " - " + ex.Message + "\n\r");
                }
            }
        }
    }
